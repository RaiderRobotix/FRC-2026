...
public class Vision extends SubsystemBase{
    ...
    private double distanceval = 0.0;
    ...
    public void periodic(){
        ...
        distanceval = updateDistance(checkForTarget(targets));
        ...
        SmartDashboard.putNumber("New Distance Val", distanceval);
        if(checkForTarget(targets) != -1){
            lockedOn = true;
        }else{
            lockedOn = false;
        }
        SmartDashboard.putBoolean("LOCKED ONTO TARGET", lockedOn);
    }
    ...
    public int checkForTarget(List<PhotonTrackedTarget> targets){
        ...
        for(int i = 0; i < targets.size(); i++){
            if(targets.get(i).getFiducialId() == Constants.AprilTagLocations.blueCenterOutpostMiddle){
                return i;
            }
        }
    ...
    public double updateTargetYaw(List<PhotonTrackedTarget> targets, int id){
        ...
        double num = targets.get(id).getYaw();
        return num;
    }
    ...
    public double updateDistance(int id){
        if(id == -1){
            return 0.0;
        }
        Transform3d transform = targets.get(id).getAlternateCameraToTarget();
